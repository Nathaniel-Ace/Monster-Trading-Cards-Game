...
public class RequestBuilderTest {
    ...
    @Test
    void testBuildRequestWithNoHeaders() throws Exception {
        BufferedReader reader = mock(BufferedReader.class);
        when(reader.readLine()).thenReturn("GET /echo/mehr?p=23 HTTP/1.1", "");
        ...
        assertEquals("/echo", request.getServiceRoute());
        assertEquals("mehr", request.getPathParts().get(1));
        assertEquals(0, request.getHeaderMap().getContentLength());
        assertNull(request.getBody());
    }

    @Test
    void testBuildRequestWithQueryParameters() throws Exception {
        BufferedReader reader = mock(BufferedReader.class);
        when(reader.readLine()).thenReturn("GET /echo/mehr?p=23 HTTP/1.1", "");
        ...
        assertEquals("/echo", request.getServiceRoute());
        assertEquals("mehr", request.getPathParts().get(1));
        assertEquals("p=23", request.getParams());
    }
